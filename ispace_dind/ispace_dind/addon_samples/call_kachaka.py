    ...
    def call_kachaka(self, data):
        ...
        for kalman in data['update_kalmans']:
            ...
            if kalman.tracker_id == self.tracked_id:
                cv2.rectangle(frame, tuple(map(int, kalman.get_bbox()[0:2])), tuple(map(int, kalman.get_bbox()[2:4])), (0, 0, 255), 3)
            ...
            if self.tracked_id == -1:
                ...
                if kalman.tracker_id == self.tracked_id:
                    ...
                    if self.count > self.TRACK_CHECK_COUNT:
                        self.count = 0
                        if np.sqrt((self.goal_x-kalman.ekf.x[0])**2+(self.goal_y-kalman.ekf.x[1])**2) > 1.0:
                            self.goal_x = kalman.ekf.x[0]
                            self.goal_y = kalman.ekf.x[1]
